...
class App:
    def __init__(self):
        ...
        self.size = self.width, self.height = int(1600 * self.sc), int(
            820 * self.sc
        )  # og: 1600 x 820, test: 2000 x 900

        self.map = None
        self.map_pos = (0, int(0.12 * self.height))
        self.map_size = self.mapw, self.maph = int(0.771 * self.width), int(
            0.87805 * self.height
        )
        ...
        self.toolbar_size = self.toolbarw, self.toolbarh = int(1.01 * self.width), int(
            0.122 * self.height
        )
    ...
    def on_init(self):
        ...
        self.map = pygame.Surface.subsurface(
            self._display_surf, (self.map_pos, self.map_size)
        )
        self.toolbar_base = pygame_gui.core.UIContainer(
            pygame.Rect(self.toolbar_pos, (self.toolbarw, self.toolbarh + 200)),
            manager=self.manager,
            starting_height=0,
        )
        self.toolbar_ui = pygame_gui.elements.UIPanel(
            pygame.Rect((0, 0), self.toolbar_size),
        ...
        self.toolbar_buttons = {
            "planner_select": pygame_gui.elements.UIDropDownMenu(
                self.planners,
                self.default_planner,
                pygame.Rect((20, 15), (250, 50)),
                manager=self.manager,
                container=self.toolbar_base,
                expansion_height_limit=100,
            ),
            "add_obs": pygame_gui.elements.UIButton(
                relative_rect=pygame.Rect((350, 12), (250, 50)),
                text="Add Obstacles",
                manager=self.manager,
            ...
            "generate_obs": pygame_gui.elements.UIButton(
                relative_rect=pygame.Rect((680, 12), (250, 50)),
            ...
            "reset_obs": pygame_gui.elements.UIButton(
                relative_rect=pygame.Rect((1010, 12), (250, 50)),
        ...
        self.option_ui_panel = pygame_gui.elements.UIPanel(
            relative_rect=pygame.Rect(self.optionp_pos, self.optionp_size),
            manager=self.manager,
            element_id="option_panel",
        ...
        self.option_title = pygame_gui.elements.UILabel(
            relative_rect=pygame.Rect(20, 20, 146, 40),
            text="General Options",
            manager=self.manager,
            container=self.option_ui_panel,
            object_id="header",
        )
        self.obstacle_textbox = pygame_gui.elements.UILabel(
            relative_rect=pygame.Rect(20, 60, 180, 35),
            text=f"Generate {self.num_obstacles} Obstacles",
            manager=self.manager,
            container=self.option_ui_panel,
        )
        self.obstacle_slider = pygame_gui.elements.UIHorizontalSlider(
            relative_rect=pygame.Rect(25, 95, 250, 40),
        ...
        self.pf_ui_options = {
            "title": pygame_gui.elements.UITextBox(
                ...
                pygame.Rect(11, 17, 268, 61),
                manager=self.manager,
                container=self.option_ui_windows[State.PF],
            ),
            "virtual_textbox": pygame_gui.elements.UITextBox(
                f'Virtual Potential Field: {self.pf_options["virtual"]}',
                pygame.Rect(11, 110, 268, 40),
                manager=self.manager,
                container=self.option_ui_windows[State.PF],
            ),
            "virtual_button": pygame_gui.elements.UIButton(
                pygame.Rect(10, 150, 270, 40),
                "Enable/Disable Virtual Field",
                manager=self.manager,
                container=self.option_ui_windows[State.PF],
            ),
        }
    ...
    def renderState(self):
        if self.state == State.PF:
            for obs in self.obstacles:
                pygame.draw.circle(
                    self.map, Color.LIGHT_PURPLE, (obs.x, obs.y), obs.rad, width=0
                )
